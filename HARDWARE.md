...
Architecture: **ESP32 (deauth) + Raspberry Pi 5 (interceptor brain) + laptop (console)**.
Roles:
- **ESP32** - transmits 802.11 deauth to knock the pilot's phone off the drone's WiFi (the one radio job the Pi/laptop can't do).
- **Pi 5** - detects, triggers the ESP32, joins the freed drone link, sends LAND; hosts the backend.
...
## 1. ESP32 (deauth module)
- [ ] Identify the exact variant (ESP32 / S2 / S3 / C3) and USB-serial chip (CP210x vs CH340) - installs the matching Windows/Linux driver.
- [ ] Flash **ESP32 Marauder** firmware (has a serial CLI so the Pi can drive it headless). Web-flasher or esptool.
...
- [ ] Prefer **targeted** deauth (phone's MAC / the AP's clients) over broadcast, so it does not also kick the Pi.
- [ ] Note **MAC randomization**: modern phones use a random MAC per SSID - capture the phone's actual MAC on the drone network so the deauther can target it.
...
- [ ] Add the Pi user to the **dialout** group (serial access to the ESP32 over USB).
...
- [ ] Power: Pi 5 needs a **5V / 5A USB-C PD** supply (or equivalent power bank) for field use; the ESP32 is powered from a Pi USB port.
...
- ESP32 deauth vs **MAC randomization** and **PMF/802.11w** (if the drone AP enables Protected Management Frames, deauth is blocked - verify the AP does NOT).
